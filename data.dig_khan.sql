...
/* Average Sales by Studio */
SELECT Studio, AVG(Worldwide) AS Average_Sales
FROM sql_practice.topmovies
GROUP BY Studio;

/* Top Grossing Movies by Year */
SELECT title, studio, MAX(Worldwide), `year`
FROM sql_practice.topmovies
GROUP BY `year`
ORDER BY `year`;

/* Lowest Grossing Movies by Year */
SELECT title, studio, MIN(Worldwide), `year`
FROM sql_practice.topmovies
GROUP BY `year`
ORDER BY `year`;

/* Movies that appealed to Domestic market */
SELECT title, studio, `year`
FROM sql_practice.topmovies
WHERE Overseas < Domestic
ORDER BY `year`;

/* Studios that have made multiple blockbuster movies  in the year */
SELECT studio, COUNT(Title), `year`
FROM sql_practice.topmovies
GROUP BY Studio, `year`
HAVING COUNT(Title) > 1
ORDER BY `year`;

/* Planning movie schedule based on Ranking */
SELECT `Rank`, title, 
    CASE
        WHEN `Rank` <= 20 THEN "Must Watch"
        WHEN `RanK` > 20 AND `Rank` <= 50 THEN "Start after top 20"
        ELSE "Watch at your own comfort"
    END AS movie_schedule
FROM sql_practice.topmovies;
